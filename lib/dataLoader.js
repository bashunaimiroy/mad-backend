...
class DataLoader {
    ...
    getIDs(genre, searchterm) {
        ...
        // note that if there's a genre, it will overrule the searchterm
        if (genre === "all") {
            if (searchterm) {
                queryString = knex("bands").select("band_id","band_genre").where("pending", "0").andWhereRaw(`MATCH(band_name) AGAINST(?)`, searchterm).toString()
            } else {
                queryString = knex("bands").select("band_id","band_genre").where("pending", "0").toString()
            }
        }
        else {
            queryString = knex("bands").select("band_id","band_genre").where("pending", "0").andWhere("admin_genre", genre).toString()
        }
        console.log('querying with string: ',queryString)
        return this.query(queryString)
    }

    getSubGenres(genre, searchterm) {
        let queryString;
        console.log("dataloader is retrieving subgenres for genre: ",genre,'and search term: ',searchterm)
        // note that if there's a genre, it will overrule the searchterm
        if (genre === "all") {
            if (searchterm) {
                queryString = knex("bands").select("band_genre").where("pending", "0").andWhereRaw(`MATCH(band_name) AGAINST(?)`, searchterm).toString()
            } else {
                queryString = knex("bands").select("band_genre").where("pending", "0").toString()
            }
        }
        else {
            queryString = knex("bands").select("band_genre").where("pending", "0").andWhere("admin_genre", genre).toString()
        }
        console.log('querying with string: ',queryString)
        return this.query(queryString)
    }
    ...
    editBand(band_id, bandObj){
        console.log('editing band with object',bandObj)
        let updateString = knex("bands")
    ...
    deleteBand(band_id){
        console.log('deleting band with id',band_id)
        let deleteString = knex('bands')
            .where({band_id:band_id})
            .del()
            .toString()
        
        return this.query(deleteString)
    }
